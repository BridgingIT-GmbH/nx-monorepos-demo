...
@Component({
  ...
  template: `
    <div *ngIf="user"
      ...
      <div class="h-40 relative bg-cover bg-center" style="background-image: url({{user.bannerpic}})">
        <img [alt]="user.name" class="left-12 w-32 h-32 absolute -bottom-16 rounded-full border-sky-700 border-2"
             [src]="user.profilepic">
      </div>
      ...
      <div class="flex px-12 pt-14">
        <div class="text-sky-700 flex flex-col items-start justify-start">
          ...
          <div class="text-slate-800 text-xs pt-1">
            <span><span class="font-bold">{{user.following.length}}</span> folge ich, </span>
            <span><span class="font-bold">{{user.follower.length}}</span> folgen mir</span>
          </div>
        </div>
      </div>
      <div class="px-12">
      ...
      <div class="px-12 pb-10 text-xs flex justify-between">
        <a class="underline text-sky-700 underline-offset-2" href="{{user.url}}" target="_blank">{{user.url}}</a>
